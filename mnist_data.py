...
'''
modified for the purpose of vae demo
also added functionality to take the integral and derivative of mnist images
'''
import gzip
import os
import urllib
...
class DataSet(object):
  ...
  def shuffle_data(self):
    ...
    self._images = self._images[perm]
    self._labels = self._labels[perm]
  def diff_batch(self, batch):
    # perform 2d differentiation on mnist image
    m2 = np.array(batch) # makes a copy
    m2[:,1:,1:,:] = m2[:,1:,1:,:]-m2[:,0:-1,1:,:]
    m2[:,1:,1:,:] = m2[:,1:,1:,:]-m2[:,1:,0:-1,:]
    return m2
  def integrate_batch(self, batch):
